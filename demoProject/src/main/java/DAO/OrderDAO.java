...
public class OrderDAO {
    ...
    public ArrayList<Order> getAllOrderOddImageForUser(int idUser) {
        ...
        try {
            connection = Connect.getConnection();
            String sql = "select idOrder, idOddImage,receiver, phoneNumber, quantity, address, status , purchareDate ,totalPrice from oddImageOrder where idUser = ?";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
    ...
    public  boolean deleteOddImageOrder(String idOrder){
        ...
        try{
            connection = Connect.getConnection();
            String sql = "delete from OddImageOrder where idOrder = ?";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            preparedStatement.setString(1,idOrder );
            int check = preparedStatement.executeUpdate();
            if(check > 0){
